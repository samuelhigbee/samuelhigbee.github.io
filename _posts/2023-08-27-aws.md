...
While you're at it, it's a good idea to set up an IAM 
account so you don't have to do everything as the root user.

You'll also have to put in a service quota increase request.
By default, new users do not have access to GPUs, likely to 
prevent accidental charges.
To do this, once you've logged in you can click on your username in the upper right 
...
The G instances are the most basic instances with GPU access.
...
Next we install the AWS command line tool.
...
file is used to configure your shell.
